...
/**********************边栏---折叠菜单**********************/
$(".menu").click(function(){
   $(this).next("ul").stop(true,true).slideToggle(500).siblings("ul").slideUp(500);
});
	
/**********************右边选项卡**********************/
$("#nav span").click(function(){
	$("#nav span").eq($(this).index()).addClass("nav_show").siblings().removeClass('nav_show');
});
/**********************右边iframe**********************/
/*$(".LeftNav").click(function(){
    $('#ConShow').attr('src',$(this).attr('ul'));
});*/
/************************************点击左边栏隐藏或显示*************************************/
$('.framehide').click(function(){
	if ($('#sidebar').css("display")=="block"){
		$('#sidebar').animate({width: 'toggle'}); //滑动向左隐藏
		$('#section').animate({ marginLeft:parseInt($('#section').css('marginLeft'),0)==0 ? -$('#section').outerWidth() : 0 });//滑动向左非隐藏
		$(".framehide").css("backgroundImage","url(img/show_icon.png)"); 
	} else {
		$('#sidebar').animate({width: 'toggle'});//滑动向右隐藏
		$('#section').animate({ marginLeft:parseInt($('#section').css('marginLeft'),'214px')==214 ? -$('#section').outerWidth() : '214px' });//滑动向非右隐藏
		$(".framehide").css("backgroundImage","url(img/hide_icon.png)"); 
	}
});
/**********************下拉框**********************/
$('.select').selectlist({
...
$(function(){
	$(".details").mouseover(function(){
		$(this).attr("src","img/content02.png");
	});
	$(".details").mouseout(function(){
		$(this).attr("src","img/content.png");
	});
});
/**********************弹出层**********************/
$(function(){
...
/******右边导航读取左栏的子菜单的名称&点击左栏子菜单右边内容处显示对应的内容*******/
window.onload = function (){
//	var oAside = document.getElementById('sidebar');
//	var aP = oAside.getElementsByTagName("p");
//	var aLi = oAside.getElementsByTagName("li");
//	var oNav = document.getElementById('nav');
//	var aB = oNav.getElementsByTagName("b");
//	var aContent = document.getElementsByClassName("content");
//	var oldContent = null;
//	var oldLi = null;
//	var num = 0;
//	oldLi = aLi[num]; //上一个li 就是第1个
//	oldContent = aContent[num];
//	
//	aContent[num].style.display = "block"; 
//	for (var i=0; i<aLi.length; i++){
//		aLi[i].index  = i;
//		aLi[i].onclick = function () {
//			aB[0].innerHTML = this.innerHTML;  //右边导航读取左栏的子菜单的名称 
//			for (var i=0; i<aLi.length; i++){ 
//			  aContent[i].style.display = "none"; //所有 aContent 都隐藏
//			  aContent[this.index].style.display = "block"; //点击当前的显示
//			  
//			  //清空上一个，当前添加样式（左栏子菜单）
//			  oldLi.className = "";
//			  oldLi = this;
...
function fnTime () {
	...
	str = iYear+ '年' +toTwo(iMonth)+'月'+toTwo(iDate)+'日'; 
	$('#time').html(str);
}
function toTwo ( n ) {
   return n < 10 ?  '0' + n : '' + n;
...
(function(obj) {
    ...
    var defaultOpt = {
        ...
        width: '300px', //宽度（px、pt、%）
        height: '270px', //高度（px、pt、%）
    ...
    function _buildAlertInfo(opt) {
        ...
        var params = _initParams(opt);
        params.width = defaultOpt.width;
        params.height = defaultOpt.height;
        params.esc = true;
        params.ok = true;
 ...
 * 重写confirm确认框
 * @param content  提示的内容
 * @param flag   自定义标记，确认框的标记
 */
function _confirm(content , fun) {
    zeroModal.confirm(content, function() {
    	fun();
    });
}
/**
 *  重写alert对话框，用于提示成功或成败
 * @param content  提示的内容
 * @param flag  成功：true 失败 : false
 */
function _alert(content,flag) {
	if(flag){
		zeroModal.success(content);
	}else{
		zeroModal.error(content);
	}
}
/**
 * 操作提示
 ...
 * 操作提示 
 * @param content 提示的标题
 * @param contentDetail  详细的内容
 * @param flag  不同操作提示的自定义标记
 */
function _alert2(content,contentDetail,fun) {
    zeroModal.alert({
        content: content,
...
/*******************模板分類************************/
$(function(){
    $("#design_classification").click(function(){
       $("p").slideToggle(500);
	   $(this).toggleClass('design_first');
	   $("p").next("ul").slideUp(500);
	   $("p").addClass('subdesign_choose');
	});
	$('.subdesign_choose').click(function(){
		var ul = $(this).next("ul");
		ul.stop(true,true).slideToggle(500);
		
		$(this).toggleClass('subdesign_choose');
	})

	$(".design_first").click(function(){
	   var ul = $(this).next("ul");
	   ul.slideToggle(500);
	   $(this).toggleClass('design_firstLess');
	});

	 $("#design_classification3").click(function(){
       $(this).next("ul").slideToggle(500);
	   $(this).toggleClass('design_first');
	});
	 $("#choose_color").click(function(){
       $(this).next("ul").slideToggle(500);
	   $(this).toggleClass('design_color');
	});
});
//颜色选择
$(function(){
	...
	//单选框双击取消选中
	$(".design_radio").click(function(){
		var that = $(this)[0];
		var c = that.c;
		$(".design_radio").each(function(){
			var desRadio = $(this);
			if(desRadio.attr("name") == $(that).attr("name")){
				desRadio[0].c=0;
				desRadio[0].checked=0;
			}
		});
		...
		$("input[type=radio]:checked").each(function(){
			checkedContent += "&nbsp;" + $(this).next().html();
		});
		$("#show").html(checkedContent);
	});
});
/**********************************tr滑过变颜色**********************************************/
$(function(){
	$(".publicTable .add").on("click",function(){
		if($(this).attr("isShow") == true || $(this).attr("isShow") == "true"){
			$(this).attr("isShow", false);
			$(this).parents("tr").eq(0).find("td").removeClass("trhover");
		}else{
			$(this).attr("isShow", true);
			$(this).parents("tr").eq(0).find("td").not(".notyou").attr("class", "trhover");
		}
	})
	 $('.management .placeTr').bind({ 
		 mouseenter: function(e) { 
			 $(this).find("td").not(".notyou").addClass("tdhover");
		 } ,
